...
- **`internal/artifact`** — the vendor-agnostic artifact model: `Kind`
  ...
  `security.go` scans a hook/mcp `command`: `LintSecurityNotice` (it runs shell code —
  informational, never fails `--strict`) versus `LintSecurityRisks` (hostile-looking
  shapes — these count).
- **`internal/project`** — `agentworks.yaml` (the project manifest: name, targets, an
  optional `eval` block), `Init` (scaffold a new project, plus the embedded `AGENTS.md` and
...
- **`internal/targets`** — the static vendor registry (which artifact kinds each vendor
  ...
  *not* a shared package like `mcpconfig`: the vendors' `hooks.json` shapes genuinely
  differ (Claude Code nests an extra matcher array per event and uses `command`; Copilot
  is flatter and uses `bash`), so a shared struct would fight the schema rather than
  reflect it. Hook events are vendor-shaped, not translated: a hook targeting Cursor uses
  Cursor's event names.
...
- **`internal/mcpclient`** — a minimal MCP client over the stdio JSON-RPC transport:
  `StartProcess` runs `mcpconfig.CommandLine` via `sh -c` as a child process and wires up
  `initialize`/`notifications/initialized`, `tools/list` (cursor pagination), and
  `tools/call`. `Probe` is the one-shot handshake + `tools/list` used by `agentworks test`
  as the smoke check for an mcp artifact with no `test:`. `OnTrace`/`OnStderr` expose the
  raw wire traffic. Out of scope: resources, prompts, sampling, roots, and the http/sse
  transports — so `agentworks run` and the smoke test only handle local stdio servers.
- **`internal/inspector`** — the Bubble Tea UI behind `agentworks run`: a filterable tool
  ...
  behind a generic protocol error. A separate package from `internal/tui`: a different
  lifecycle (connect once, then browse/call/inspect until quit).
