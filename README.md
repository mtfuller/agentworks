...
| --- | --- |
| `agentworks init [path]` | Scaffold a new project (`agentworks.yaml`, the 5 kind directories, and an `AGENTS.md` + `.agents/skills/agentworks-cli/SKILL.md` teaching a coding agent how to drive this CLI in the project). Pass `--target` (repeatable) to set the project's default vendor targets, or leave it out in an interactive terminal for a prompt -- every later `agentworks new`, CLI or TUI, pre-fills a new artifact's targets from this default instead of asking from scratch each time (still overridable per artifact). |
...
| `agentworks tui` | Full-screen Bubble Tea browser: a tab per artifact kind (`tab`/`shift+tab` to switch) showing that kind's artifacts directly, drill into one with `enter` for its rendered frontmatter and body. Press `n` to scaffold a new artifact (the same wizard `agentworks new` uses, pre-filled with the current tab's kind and the project's default targets), `e` to export the current one to a vendor target with a zip toggle, `t` to run its declared `test:` command, `a` to search and import from agentskills.codes plus the Claude Code and GitHub Copilot marketplaces, or `b` to browse/search the built-in starter templates (also tabbed by kind) and create straight from one — all run right there, no dropping back to the CLI. |
